...
{
	  int i=0;
		if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)//进入 接收中断
	  {
		    usart_ucTemp=USART1->DR;//读取接收到的数据
        if(usart_ucTemp=='$')    //接收到开始字符
				{
		...
		if(usart_Rx_data_flag)
		{
					...
					else 
         {
			...
			/* 遇到换行字符，认为接收到一个命令 */
			if (usart_ucTemp == 0x0A && usart_Rx_data_flag)	/* 换行字符 */
			{		
...
//			
//			/* 遇到换行字符，认为接收到一个命令 */
//			if (usart_ucTemp == 0x0A && usart_Rx_control_flag)	/* 换行字符 */
//			{		
